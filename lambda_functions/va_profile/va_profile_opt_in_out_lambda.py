...
if NOTIFY_ENVIRONMENT == "test":
    ssl_context = None
elif CERTIFICATE_ARN is None:
    logger.error("CERTIFICATE_ARN is not set.")
    ssl_context = None
else:
    decryption_password = ''.join(choices(ascii_letters, k=100)).encode()

    try:
        ssl_context = ssl.create_default_context(cafile="/opt/va_ca_chain.pem")

        # Get the client certificates from AWS ACM.
        logger.debug("Making a request to ACM . . .")
        acm_client = boto3.client("acm")
        response: dict = acm_client.export_certificate(
            CertificateArn=CERTIFICATE_ARN,
            Passphrase=decryption_password
        )
        logger.debug(". . . finished the request to ACM.")

        with NamedTemporaryFile() as f_public_chain, NamedTemporaryFile() as f_private_key:
            f_public_chain.write(response["Certificate"])
            f_public_chain.write(response["CertificateChain"])
            f_private_key.write(response["PrivateKey"])

            ssl_context.load_cert_chain(f_public_chain.name, f_private_key.name, decryption_password)
    except (OSError, ClientError, ssl.SSLError) as e:
        logger.exception(e)
        ssl_context = None
